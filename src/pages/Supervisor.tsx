...
export default function Supervisor() {
  ...
  const { data: allAssignments = [], refetch } = useQuery({
    queryKey: ['trainer-assignments', userId],
    queryFn: async () => {
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Manage your trainer sign-offs.</p>
          </div>
          <Button 
            variant="outline" 
            size="sm" 
                  ...
                  return (
                    <div key={a.id} className="flex items-center justify-between border rounded-lg p-4">
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">Employee: {a.user?.first_name || 'N/A'} {a.user?.last_name || ''} • {a.user?.email || 'N/A'}</div>
                        {hasCompletion && (
